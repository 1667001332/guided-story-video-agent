...
→ 完整故事 → 用户确认 → 自适应场景剧本 → 用户确认
→ 视觉圣经 → 内容驱动的动态分镜 → 用户确认费用 → 视频生成
```

v0.4 取消了固定五节点大纲、固定五场剧本和固定 5–10 镜头模板。故事先作为独立产物生成、修改和确认；只有确认后的故事才能改编剧本。剧本场景和分镜数量由内容决定，时长只作为制作能力约束。每个镜头保存首帧、连续动作、结束帧和参考资产 ID，为后续参考图生视频保留稳定接口。
...
覆盖一句话 8 卡、选择与混合来源、可选故事零件、故事版本、故事确认门禁、动态剧本场景、内容驱动分镜、视觉圣经、三段式镜头 Prompt、失败镜头续跑、schema 迁移、人工 CLI、Gradio `process_api` 和付费门禁。
