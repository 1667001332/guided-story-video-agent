...
本指南用于 `v0.7.0` 的真实 API 验收。请使用测试者自己的 API Key，不要接收或复制项目作者的 `.env`。
...
批量测试默认是“严格文本、关闭视频”：故事、剧本或分镜规划只要发生请求失败、
JSON 解析失败或结构校验失败，就会保留错误并判定该次失败，不再静默生成离线替代
成品；测试不会调用视频 API。每条用例完成后都会立即保存结果，因此中断后可以继续：
